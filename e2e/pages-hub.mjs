...
try{
 ...
 assert.equal(await page.locator('.game-link').count(),5);
 const links=await page.locator('.game-link').evaluateAll(nodes=>nodes.map(a=>a.href));
 assert.deepEqual(links,['https://spencerrichardhenry.github.io/wildtag/tiny-tide.html','https://spencerrichardhenry.github.io/wildtag/royal-yeet.html','https://spencerrichardhenry.github.io/mobatest/','https://spencerrichardhenry.github.io/wildtag/','https://spencerrichardhenry.github.io/moss-and-maw/']);
 ...
 assert.deepEqual(errors,[]);writeFileSync(`${output}/hub-results.json`,JSON.stringify({passed:true,links,errors},null,2));console.log('PASS: five game links, loaded previews, keyboard access, desktop and 390/320 px mobile layouts.');

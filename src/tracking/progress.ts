...
// critter is inside the ring and decays at half rate (−dt/2) while outside, so
// a brief loss of proximity is forgiving but a fleeing critter slips away.
// Clamped to [0, trackTime]. No three, no DOM — trivially unit-testable.
// ---------------------------------------------------------------------------
 ...
 * outside it falls by `dt/2`. Result is clamped to `[0, sp.trackTime]`.
 */
export function stepTracking(
  ...
  const delta = dist <= sp.trackRadius ? dt : -dt / 2;

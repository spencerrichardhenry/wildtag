...
the puzzle. It is a game about patience, traversal and the quiet thrill of
getting close to something wild without scaring it off.
...
Eight procedural critters, ordered easiest → hardest. Difficulty rises with
awareness radius, flee speed/style and the tracking time needed to Link.

| Species       | Biome(s)                     | Flees by       | Difficulty |
| ------------- | ---------------------------- | -------------- | ---------- |
| Puffle        | Meadow                       | stands ground  | Easy       |
| Skitterling   | Meadow, Forest               | sprints        | Easy       |
| Bellowbuck    | Forest                       | stands ground  | Medium     |
| Mirefin       | Wetland, Water               | dives & swims  | Medium     |
| Craghorn      | Crags                        | scales ledges  | Medium     |
| Zephyrfinch   | Meadow, Forest, Highlands    | takes flight   | Hard       |
| Emberpup      | Highlands                    | zigzags        | Hard       |
| Lumen Stag    | Forest, Highlands, Crags     | sprints        | Legendary  |

The Lumen Stag is effectively unique world-wide (rarity ~0.02, concurrency
capped) — a rare deep-wood trophy.
...
| Critters    | `critters/species.ts` (data), `critters/ai.ts` (pure state machine), `critters/manager.ts` (streaming world manager), `critters/models.ts`, `critters/animation.ts` |
...
| UI          | `ui/hud.ts` (+ pure `ui/hud-math.ts`), `ui/screens.ts`, `ui/guide.ts`, `ui/toasts.ts`, `ui/audio.ts` |
...
```bash
npm test         # vitest — the full pure-logic suite (258 tests)
...
| `?preview=critters` | Turntable showcase of all 8 species |
...
`window.__game` exposes a debug handle (see `src/debug.ts`): `state()`,
`player.{pos,teleport,setStamina}`, `grant(kind, n)` (plain resource fields, or a
kit via the `kit:<id>` prefix), `unlockAll()`, `spawn(speciesId, dist)`,
`track(id)`, `completeTracking(id)`, `setTimeScale(f)`, `listCritters()`,
`save()`, `reset()`.
...
grapple, save round-trip and a perf smoke — writing a screenshot per phase to
`docs/verify/`.
...
files or fonts beyond the OS monospace stack. Terrain, props, all eight critters
and their animations, the sky gradient and water, the UI and the chime are all
generated in code at runtime from a single world seed. Nothing is downloaded and
no third-party art is bundled.

## Save data

Progress autosaves to `localStorage` (`wildtag-save-v1`) every 10 seconds and on
tab close — inventory, unlocks, linked species, placed structures and your
position. A missing or malformed save always falls back to a clean fresh start
